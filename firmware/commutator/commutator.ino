...
void print_turns()
{
    // Print target turns when commutator turns
    StaticJsonBuffer<256> jbuff;
    JsonObject& doc = jbuff.createObject();
    
    // Print just the target turns, which is used for control
...
void setup()
{
    ...
    // Start the timer for printing turns every 100 ms
    print_timer.begin(print_turns, 1e5);
}
